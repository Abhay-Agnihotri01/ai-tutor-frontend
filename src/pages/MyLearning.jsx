...
import { Skeleton, CourseCardSkeleton } from '../components/common/Skeleton';
...
const MyLearning = () => {
  ...
  const handleRateClick = async (course) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/ratings/course/${course.id}/user`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
            ...
            <div className="theme-card p-4 rounded-lg mb-6">
              <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${filterBy === 'all'
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${filterBy === 'completed'
                    ? 'bg-primary-600 text-white'
                    : 'theme-bg-secondary theme-text-primary hover:theme-bg-tertiary'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${filterBy === 'in-progress'
                    ? 'bg-primary-600 text-white'
                    : 'theme-bg-secondary theme-text-primary hover:theme-bg-tertiary'
                    }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors ${filterBy === 'not-started'
                    ? 'bg-primary-600 text-white'
                    : 'theme-bg-secondary theme-text-primary hover:theme-bg-tertiary'
                    }`}
                >
                ...
                {getUniqueCategories().map(category => (
                  <button
                    ...
                    className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors capitalize ${filterBy === category
                      ? 'bg-primary-600 text-white'
                      : 'theme-bg-secondary theme-text-primary hover:theme-bg-tertiary'
                      }`}
                  >
                  ...
                  <div key={enrollment.id} className="theme-card rounded-lg overflow-hidden hover:shadow-lg transition-all duration-300 hover:scale-105">
                    <div className="aspect-video relative overflow-hidden bg-gradient-to-br from-primary-400 to-primary-600">
                      ...
                      <div className="absolute top-4 right-4">
                        <div className={`px-2 py-1 rounded text-xs font-medium shadow-sm ${isCompleted
                    ...
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-2">
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full ${course?.level === 'beginner' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' :
                      ...
                      <div className="mb-4">
                        ...
                        <div className="w-full bg-gray-200 dark:bg-gray-700 rounded-full h-2 shadow-inner">
                          <div
                            className={`h-2 rounded-full transition-all duration-300 ${isCompleted ? 'bg-green-500' : 'bg-primary-600'
                              }`}
                      ...
                      <div className="flex space-x-2">
                        ...
                        >
                          ...
                          <span>{isCompleted ? 'Revisit Course' : 'Continue Learning'}</span>
                        </Link>
                        <Link
