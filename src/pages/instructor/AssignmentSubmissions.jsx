...
const AssignmentSubmissions = () => {
  ...
  const fetchSubmissions = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/quiz/submissions/${quizId}`, {
        headers: {
              ...
              <tbody className="divide-y theme-border">
                {submissions.map((submission) => (
                  <tr key={submission.id} className="hover:theme-bg-secondary">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${submission.status === 'graded'
