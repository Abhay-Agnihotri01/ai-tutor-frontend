...
// import RecordingManager from '../../components/live/RecordingManager';
// import RecordingUploader from '../../components/instructor/RecordingUploader';
// import RecordingsList from '../../components/instructor/RecordingsList';
...
const CourseBuilder = () => {
  ...
  //   try {
  //     const response = await fetch(`${API_URL}/live-classes/course/${id}`, {
  ...
  const fetchCourseData = async () => {
    try {
      const [courseResponse, chaptersResponse] = await Promise.all([
        ...
        fetch(`${API_URL}/chapters/courses/${id}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        })
      ]);
  ...
  const fetchQuizzes = async () => {
    try {
      ...
      for (const chapter of chapters) {
        ...
        const response = await fetch(`${API_URL}/quiz/chapter/${chapter.id}?cb=${timestamp}`, {
          headers: {
  ...
  const handleChapterSubmit = async () => {
    ...
    try {
      if (editingChapter) {
        ...
        const response = await fetch(`${API_URL}/chapters/${editingChapter}`, {
          method: 'PUT',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const handleResourceSubmit = async () => {
    ...
    try {
      ...
      const url = editingResource
        ? `${API_URL}/resources/${editingResource}`
        : `${API_URL}/resources`;
    ...
    if (editingVideo) {
      ...
      if (videoForm.file) {
        ...
        try {
          ...
          const response = await fetch(`${API_URL}/videos/${editingVideo}/replace`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`
        ...
        // Update only title
        try {
          const response = await fetch(`${API_URL}/videos/${editingVideo}`, {
            method: 'PUT',
            headers: {
              'Authorization': `Bearer ${localStorage.getItem('token')}`,
              'Content-Type': 'application/json'
  ...
  const updateChapterOrder = async (chapters) => {
    ...
    await fetch(`${API_URL}/chapters/reorder`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
  ...
  const updateResourceOrder = async (resources) => {
    ...
    const response = await fetch(`${API_URL}/resources/reorder`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
  ...
  const updateVideoOrder = async (videos) => {
    ...
    const response = await fetch(`${API_URL}/videos/reorder`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
  ...
  const updateTextLectureOrder = async (textLectures) => {
    ...
    const response = await fetch(`${API_URL}/text-lectures/reorder`, {
      method: 'PUT',
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`,
        'Content-Type': 'application/json'
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="lg:col-span-3">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      if (item.uploadType === 'url') {
                                        ...
                                        window.open(`${BASE_URL}${item.filePath}`, '_blank');
                                      }
                                    }}
                                  ...
                                  <Button
                                    ...
                                    onClick={async () => {
                                      if (window.confirm('Delete this text lecture?')) {
                                        try {
                                          const response = await fetch(`${API_URL}/text-lectures/${item.id}`, {
                                            method: 'DELETE',
                                            headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
                                          });
                                          if (response.ok) {
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                if (window.confirm('Delete this quiz?')) {
                                  try {
                                    const response = await fetch(`${API_URL}/quiz/quizzes/${quiz.id}`, {
                                      method: 'DELETE',
                                      headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
                                    });
                                    if (response.ok) {
            ...
            {/* <div className="theme-card p-6 rounded-lg mt-6">
              <RecordingUploader
                courseId={id}
                onUploadSuccess={() => setRecordingsRefreshTrigger(prev => prev + 1)}
              />
              <div className="mt-6">
                <h3 className="font-semibold theme-text-primary mb-3">Uploaded Recordings</h3>
                <RecordingsList
                  courseId={id}
                    ...
                    return (
                      <div key={liveClass.id} className="p-3 theme-bg-secondary rounded-lg border theme-border">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`px-2 py-1 rounded-full text-xs font-medium ${isLive ? 'bg-red-100 text-red-700 dark:bg-red-900 dark:text-red-300' :
                        ...
                        <div className="flex space-x-1">
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  if (!token || token === 'null' || token === 'undefined') {
                                    toast.error('Please log in to start live classes');
                                    return;
                                  }

                                  const response = await fetch(`${API_URL}/live-classes/${liveClass.id}/start?cb=${Date.now()}&t=${Math.random()}`, {
                                    method: 'PATCH',
                                    headers: {
                                  ...
                                  console.log('CourseBuilder start live class response status:', response.status);
                                  console.log('CourseBuilder start live class response headers:', response.headers);

                                  if (response.ok) {
                                    await fetchLiveClasses();
                                    console.log('Live class meetingId:', liveClass.meetingId);
                                    const meetingUrl = `/instructor/live-class/${liveClass.meetingId}`;
                                    ...
                                    console.log('CourseBuilder start live class error data:', errorData);

                                    if (response.status === 401) {
                                      toast.error('Authentication failed. Please log in again.');
                                    } else if (response.status === 403) {
                                      toast.error('Access denied. Only the instructor can start this live class.');
                                    } else if (response.status === 404) {
                                      toast.error('Live class not found.');
                                    } else {
                                      toast.error(`Error ${response.status}: ${errorData.message || 'Failed to start live class'}`);
                                    }
                                  }
                                } catch (error) {
                          ...
                          {isLive && (
                            <Button
                              size="sm"
                              onClick={() => window.open(`/instructor/live-class/${liveClass.meetingId}`, '_blank')}
                          ...
                          {isLive && (
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  const response = await fetch(`${API_URL}/live-classes/${liveClass.id}/end`, {
                                    method: 'PATCH',
                                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
                                  });
                                  if (response.ok) {
                          ...
                          <Button
                            ...
                            onClick={async () => {
                              if (window.confirm('Delete this live class?')) {
                                try {
                                  const response = await fetch(`${API_URL}/live-classes/${liveClass.id}`, {
                                    method: 'DELETE',
                                    headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
                                  });
                                  if (response.ok) {
