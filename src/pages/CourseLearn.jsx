...
import { Play, CheckCircle, Clock, FileText, ChevronDown, ChevronRight, Star, Download, HelpCircle, BookOpen, Video, Award, MessageCircle } from 'lucide-react';
...
// import PublishedRecordings from '../components/live/PublishedRecordings';
import RecordingsViewer from '../components/student/RecordingsViewer';
...
const CourseLearn = () => {
  ...
  const fetchRecordings = async () => {
    try {
      const response = await fetch(`${API_URL}/upload/course/${id}`);
      if (response.ok) {
  ...
  */
  ...
  const loadVideoProgress = async () => {
    try {
      ...
      const response = await axios.get(`${API_URL}/enrollments/video-progress/${id}`, {
    ...
    try {
      ...
      for (const chapter of course.chapters) {
        try {
          ...
          const response = await axios.get(`${API_URL}/quiz/chapter/${chapter.id}?cb=${cacheKey}`, {
            headers: {
          ...
          if (response.data.success) {
            ...
            for (const quiz of response.data.quizzes) {
              try {
                const attemptResponse = await axios.get(`${API_URL}/quiz/attempt/status/${quiz.id}?cb=${cacheKey}`, {
                  headers: {
                    'Authorization': `Bearer ${token}`,
                    'Cache-Control': 'no-cache, no-store, must-revalidate',
                    'Pragma': 'no-cache',
  ...
  const startQuiz = async (quiz) => {
    ...
    try {
      const response = await axios.post(`${API_URL}/quiz/attempt/start`,
        { quizId: quiz.id },
        {
          headers: {
            'Authorization': `Bearer ${token}`
  ...
  /*
  const toggleLiveClasses = () => {
    setExpandedLiveClasses(!expandedLiveClasses);
  };
  */
  ...
  */
  ...
  const markLessonComplete = async () => {
    if (currentLesson) {
      ...
      try {
        const token = localStorage.getItem('token');
        await axios.post(`${API_URL}/enrollments/mark-content-complete`, {
          courseId: id,
  ...
  const handleRateClick = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/ratings/course/${id}/user`, {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  if (loading) {
    return (
      <div className="min-h-screen theme-bg-primary">
        <div className="theme-bg-secondary border-b theme-border py-4">
        ...
        <div className="max-w-7xl mx-auto flex h-[calc(100vh-100px)]">
          ...
          <div className="w-80 theme-bg-primary border-l theme-border p-6 space-y-6 hidden lg:block overflow-y-auto">
  ...
  return (
    <div className="min-h-screen theme-bg-primary">
        ...
        <div className="max-w-7xl mx-auto px-4 py-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center space-x-4">
              ...
              {getProgress() >= 100 && (
                <button
                  onClick={() => setCertificateModal({ isOpen: true })}
                  className="flex items-center space-x-2 bg-gradient-to-r from-yellow-500 to-orange-500 hover:from-yellow-600 hover:to-orange-600 text-white px-4 py-2 rounded-lg font-semibold transition-all duration-200 transform hover:scale-105 animate-pulse"
                >
            ...
            <div className="animate-fade-in">
              ...
              <div className="mb-6">
                <div className="flex items-center justify-between mb-4">
                  <h2 className="text-2xl font-bold theme-text-primary">{currentRecording.title}</h2>
                ...
                <div className="mb-4 flex items-center justify-between">
                  <div className="flex items-center space-x-4">
                    <div className="flex items-center text-green-600">
                      ...
                      <span className="font-medium">Live Class Recording</span>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-2 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                    <button
                      ...
                      className={`px-4 py-2 rounded-md text-sm font-semibold transition-all duration-200 flex items-center ${notesView === 'lecture'
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-md text-sm font-semibold transition-all duration-200 flex items-center ${notesView === 'student'
                        ? 'bg-white dark:bg-gray-700 text-primary-600 dark:text-primary-400 shadow-sm border border-primary-200 dark:border-primary-700'
                        : 'text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-white/50 dark:hover:bg-gray-700/50'
                        }`}
                    >
                  ...
                  {notesView === 'lecture' ? (
                    ...
                    <div>
                      <div className="flex items-center mb-4">
                        <Video className="w-5 h-5 mr-2 theme-text-secondary" />
                        <h3 className="font-semibold theme-text-primary">Recording Details</h3>
                      </div>
                      ...
                      {currentRecording.duration && (
                        <div className="mt-3 text-sm theme-text-muted">
                          Duration: {Math.floor(currentRecording.duration / 60)}:{(currentRecording.duration % 60).toString().padStart(2, '0')}
                        </div>
                      )}
                    </div>
                  ) : (
                    ...
                    <div>
                      ...
                      <StudentNotes
                        videoId={currentRecording.recordingId || currentRecording.id}
                        courseId={id}
              ...
              <div className="mb-6">
                ...
                <div className="mb-4 flex items-center justify-between">
                  ...
                  <div className="flex space-x-2 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
                    ...
                    <button
                      onClick={() => setNotesView('discussion')}
                      className={`px-4 py-2 rounded-md text-sm font-semibold transition-all duration-200 flex items-center ${notesView === 'discussion'
                        ? 'bg-white dark:bg-gray-700 text-primary-600 dark:text-primary-400 shadow-sm border border-primary-200 dark:border-primary-700'
                        : 'text-gray-600 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 hover:bg-white/50 dark:hover:bg-gray-700/50'
                        }`}
                    >
                ...
                {/* <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    ...
                    <div>
                      ...
                      >
                        <div className="flex items-center justify-between p-2 rounded-lg theme-bg-secondary hover:theme-bg-tertiary transition-all duration-200">
                          <div className="flex items-center space-x-2">
                            <span className="text-sm font-medium theme-text-primary">Recordings</span>
                          </div>
                          <div className="flex items-center space-x-2">
                      ...
                      {expandedLiveClasses && (
                        <div className="animate-slide-up">
                          <RecordingsViewer courseId={id} onSelectRecording={selectRecording} />
                        </div>
                      )}
                    </div>
                  </div>
                              ...
                              if (item.contentType === 'video') {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`w-full group transition-all duration-200 ${currentLesson?.id === lesson.id
                                  ...
                                  >
                                    <div className="flex items-center space-x-3 p-3">
                                      ...
                                      <div className="flex-1 min-w-0 text-left">
                                        <p className={`text-sm font-medium truncate transition-colors ${currentLesson?.id === lesson.id
                                  ...
                                  >
                                    <div className="flex items-center space-x-3">
                                      ...
                                      <button
                                        onClick={() => {
                                          const token = localStorage.getItem('token');
                                          const url = `${API_URL}/download/resource/${resource.id}?token=${token}`;
                                ...
                                return (
                                  <button
                                    ...
                                    className={`w-full group transition-all duration-200 ${currentTextLecture?.id === textLecture.id
                                      ? 'bg-primary-50 dark:bg-primary-900/30 border-l-3 border-primary-500'
                                      : 'hover:bg-gray-50 dark:hover:bg-gray-800/50'
                                      } rounded-r-lg`}
                                  >
                                    <div className="flex items-center space-x-3 p-3">
                                      ...
                                      {/* Text Lecture Info */}
                                      <div className="flex-1 min-w-0 text-left">
                                        <p className={`text-sm font-medium truncate transition-colors ${currentTextLecture?.id === textLecture.id
                                          ? 'text-primary-600 dark:text-primary-400'
                                          : completedLessons.has(textLecture.id)
                                            ? 'theme-text-primary'
                                            : 'theme-text-primary group-hover:text-primary-600'
                          ...
                          {quizzes[chapter.id] && quizzes[chapter.id].length > 0 && quizzes[chapter.id].map((quiz) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full rounded-lg border transition-all duration-200 ${canRetake
                              ...
                              >
                                <div className="p-4">
                                  <div className="flex items-center space-x-4 hover:bg-black/5 dark:hover:bg-white/5 transition-colors rounded-lg p-2">
                                    {/* Quiz Icon */}
                                    <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${canRetake
                                    ...
                                    <div className="flex-1 min-w-0">
                                      <div className="flex items-center justify-between mb-2">
                                        <h4 className={`text-sm font-semibold truncate ${canRetake
                                      ...
                                      <div className="flex items-center justify-between">
                                        ...
                                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${canRetake
      ...
      <CertificateModal
        ...
        onClose={() => setCertificateModal({ isOpen: false })}
        courseId={id}
        courseName={course?.title}
        onGenerate={(certificate) => {
          toast.success('Certificate generated successfully!');
          setCertificateModal({ isOpen: false });
        }}
      />
