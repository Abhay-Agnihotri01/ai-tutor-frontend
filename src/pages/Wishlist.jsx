...
const Wishlist = () => {
  ...
  const removeFromWishlist = async (courseId) => {
    try {
      const response = await fetch(`${API_URL}/wishlist/remove/${courseId}`, {
        method: 'DELETE',
        headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
      });
      const data = await response.json();
