...
const PDFViewer = ({ documentUrl, documentId, courseId, onComplete, onProgress }) => {
  ...
  const loadProgress = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/progress/document/${documentId}`, {
  ...
  const saveProgress = async (progress) => {
    try {
      const token = localStorage.getItem('token');
      await fetch(`${API_URL}/progress/document`, {
        method: 'POST',
        headers: {
  ...
  const markAsComplete = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${API_URL}/progress/document/complete`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        ...
        <div className="flex items-center space-x-2">
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center space-x-1 ${isCompleted
