...
const AllNotesModal = ({ courseId, course, isOpen, onClose, inline = false, onJumpToTime }) => {
  ...
  const loadAllNotes = async () => {
    ...
    try {
      ...
      // Load video notes (includes both regular video notes and live class recording notes)
      const videoNotesResponse = await fetch(`${API_URL}/notes/course/${courseId}`, {
      ...
      if (videoNotesResponse.ok) {
        ...
        const videoNotes = (videoData.notes || []).filter(note => {
          ...
          // Check if this is a live class recording note
          if (!note.videoId && note.content) {
            try {
              const contentObj = JSON.parse(note.content);
              if (contentObj.isLiveClassRecording && contentObj.recordingId) {
                return {
                  ...note,
                  isLiveClassRecording: true,
                  recordingId: contentObj.recordingId,
      ...
      try {
        const textNotesResponse = await fetch(`${API_URL}/notes/text-lecture/course/${courseId}`, {
          headers: { 'Authorization': `Bearer ${token}` }
        });
  ...
  const groupNotesByChapter = (notesData) => {
    ...
    notesData.forEach(note => {
      ...
      } else if (note.isLiveClassRecording) {
        // Live class recording note - create a virtual "Live Classes" section
        chapterData = {
          chapter: { id: 'live-classes', title: 'Live Classes' },
          recording: { id: note.recordingId, title: `Recording: ${note.recordingId.split('/').pop()}` }
        };
      }
      ...
      if (chapterData) {
        ...
        const contentKey = note.videoId
          ...
          : note.textLectureId
            ...
            : `recording-${chapterData.recording.id}-${chapterData.recording.title}`;
        ...
        if (!grouped[key].videos[contentKey]) {
          grouped[key].videos[contentKey] = {
            ...
            recording: chapterData.recording || null,
            notes: []
          };
        }
                      ...
                      <div key={videoKey}>
                        <h4 className="text-sm font-semibold theme-text-secondary mb-3 flex items-center">
                          ...
                          <span className="truncate flex-1">
                            {videoData.video?.title || videoData.textLecture?.title || videoData.recording?.title || 'Unknown Content'}
                          </span>
                            ...
                            <div key={`${note.id}-${noteIndex}`} className="theme-bg-secondary p-4 rounded-lg border theme-border group hover:shadow-md transition-all duration-200">
                              <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center space-x-2">
                                  ...
                                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${note.type === 'text'
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-black transition-opacity duration-300 z-40 ${isOpen ? 'bg-opacity-20' : 'bg-opacity-0 pointer-events-none'
          }`}
        onClick={onClose}
      />

      <div className={`fixed top-0 right-0 h-full w-96 bg-white dark:bg-gray-800 shadow-2xl transform transition-transform duration-300 ease-in-out z-50 ${isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}>
                            ...
                            <div key={`${note.id}-${noteIndex}-small`} className="theme-bg-secondary p-2 rounded border theme-border group hover:shadow-sm transition-shadow">
                              <div className="flex items-center justify-between mb-1">
                                <div className="flex items-center space-x-1">
                                  ...
                                  <span className={`px-1 py-0.5 rounded text-xs ${note.type === 'text'
