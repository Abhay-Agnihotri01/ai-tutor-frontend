...
import { Send, MessageSquare, Plus, Clock, AlertCircle, CheckCircle, Search, Archive, Star, MoreVertical, ArrowLeft, Paperclip, Eye, Reply } from 'lucide-react';
...
const CommunicationPanel = () => {
  ...
  const statuses = [
    { value: 'unread', label: 'Unread', icon: Clock, color: 'text-yellow-600' },
    { value: 'read', label: 'Read', icon: Eye, color: 'text-blue-600' },
    { value: 'replied', label: 'Replied', icon: Reply, color: 'text-green-600' },
    { value: 'resolved', label: 'Resolved', icon: CheckCircle, color: 'text-gray-600' }
  ];
  ...
  useEffect(() => {
    if (showNewMessage) return; // Don't auto-refresh when composing

    const interval = setInterval(() => {
      if (document.visibilityState === 'visible') {
        fetchCommunications();
        // Refresh selected conversation without state clearing
  ...
  }, [selectedCommunication?.id, showNewMessage]);
  ...
  const fetchInstructors = async () => {
    try {
      const response = await fetch(`${API_URL}/admin-communications/instructors`, {
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
      });
      const data = await response.json();
      setInstructors(data);
  ...
  const handleSendMessage = async (e) => {
    ...
    setNewMessage({ receiverId: '', subject: '', message: '', priority: 'normal', category: 'general' });
  ...
  const handleViewCommunication = async (communication) => {
    if (selectedCommunication?.id === communication.id) return; // Prevent re-loading same conversation

    // Set loading state
    setSelectedCommunication({ ...communication, loading: true });
    setReplies([]);
    ...
    try {
      ...
      setSelectedCommunication(prev =>
        prev?.id === communication.id ? { ...data.communication, loading: false } : prev
      );
      setReplies(data.communication?.replies || []);

      // Mark as read if unread (async, don't wait)
      if (communication.status === 'unread') {
        fetch(`${API_URL}/admin-communications/${communication.id}/status`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        }).then(() => fetchCommunications()).catch(() => { });
      }
    } catch (error) {
  ...
  const handleReply = async (e) => {
    e.preventDefault();
    if (!replyMessage.trim() || !selectedCommunication?.id) return;
    ...
    try {
      const response = await fetch(`${API_URL}/admin-communications/${communicationId}/reply`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${localStorage.getItem('token')}`
        ...
        setReplies(prev =>
          prev.map(reply =>
            reply.id === tempReply.id
              ? { ...realReply, sender: { name: `${user.firstName} ${user.lastName}` } }
              : reply
          )
        );
  ...
  const getStatusColor = (status) => {
    return statuses.find(s => s.value === status)?.color || 'text-gray-600';
  };
      ...
      <div className="bg-white dark:bg-gray-800 border-b dark:border-gray-700 px-6 py-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Communications</h1>
          </div>
          <button
                ...
                >
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-medium text-sm">
                      {comm.isFromAdmin ? 'A' : (comm.sender?.name?.[0] || 'U')}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <StatusIcon className={`w-3 h-3 ${getStatusColor(comm.status)}`} />
                        </div>
                      </div>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600 dark:text-gray-400">
                        {selectedCommunication.isFromAdmin
                          ? `To: ${selectedCommunication.receiver?.name || 'Instructor'}`
                          : `From: ${selectedCommunication.sender?.name || 'Unknown'}`}
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg border dark:border-gray-700 p-4">
                  <div className="flex items-start gap-3">
                    <div className="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-full flex items-center justify-center text-white font-medium text-xs">
                      {selectedCommunication.isFromAdmin ? 'A' : (selectedCommunication.sender?.name?.[0] || 'U')}
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-2">
                        <p className="font-medium text-gray-900 dark:text-white">
                          {selectedCommunication.isFromAdmin ? 'Admin' : (selectedCommunication.sender?.name || 'Unknown')}
                        </p>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
            <div className="flex items-center justify-between p-6 border-b dark:border-gray-700">
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Compose Message</h2>
              <button
