...
const QuizTaker = ({ quiz, onComplete, courseId }) => {
  const { updateQuizProgress, updateAssignmentProgress } = useProgressTracking();
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/quiz/${quiz.id}/attempts`, {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          answers: answers,
          timeTaken: quiz.timeLimit ? (quiz.timeLimit * 60) - timeLeft : timeSpent
        })
      });
  ...
  const handleAssignmentSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/quiz/assignment/submit`, {
        method: 'POST',
        headers: {
