      ...
      if (quizForm.type === 'quiz' && questions.length > 0) {
        ...
        for (let i = 0; i < questions.length; i++) {
          ...
          try {
            ...
            const questionResponse = await fetch(`${API_URL}/quiz/${quizData.quiz.id}/questions`, {
              method: 'POST',
              headers: {
                'Authorization': `Bearer ${localStorage.getItem('token')}`,
                'Content-Type': 'application/json'
                            ...
                            <div className="grid grid-cols-2 gap-1">
                              {q.options.map((opt, optIndex) => (
                                <div key={optIndex} className="flex items-center space-x-1">
                                  <span className={`w-2 h-2 rounded-full ${(Array.isArray(q.correctAnswer) ? q.correctAnswer.includes(opt) : q.correctAnswer === opt)
